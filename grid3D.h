...
#define ALPHA 0.01
#define SIGMA 5
#define THRESHOLD_CONFIDENCE 0.15
...
//            cout << "DISTANCE " << new_distance << endl;
    ...
    */
    ...
    /*!
     * function for creating GRIDCELL map from our grid, we go over each voxel of our grid
     * and check all of the possible GRIDCELLS that can contain him (each voxel has 8)
     * we check that the grid cell is within our grid boundries, and also we check
     * if he is legal (NEED TO COMPLETE), we add GRIDCELL to the gridcell map only if he hasent
     * been added yet, which is possible since a gridcell can be added by all of the voxel it contains
     * so we dont want to save replicas.
     */
//    void createGridCellMap(){
//        vector<vector<tuple<int,int,int>>> possible_grid_cells;
//        int counter = grid.size();
//        cout << counter << endl;
//        for(auto it = grid.begin(); it != grid.end(); it++){
////            if(it->second.getDistance() >= 0){
//////                cout << "**********************************" << it->second.getDistance() << "**********************" << endl;
////            }
//            findAllVertexes(it->first, possible_grid_cells);
//            for(int i = 0; i < possible_grid_cells.size(); i++){
//                gridCellKey key(possible_grid_cells[i]);
//                if(grid_cell_map.find(key) == grid_cell_map.end()){
//                    GRIDCELL new_cube(possible_grid_cells[i], this->getDensifiedGrid(), this);
//                    //new_cube.print();
//                    pair<gridCellKey, GRIDCELL> new_grid_cell(key, new_cube);
//                    grid_cell_map.insert(new_grid_cell);
//
//
//                }
...
GRIDCELL::GRIDCELL(vector<tuple<int,int,int>>& indexes_of_vertexes, map<tuple<int,int,int>, voxel>& map, grid3D* grid){
    for(int i = 0; i < indexes_of_vertexes.size(); i++){
        if(get<0>(indexes_of_vertexes[i]) == -0.5){
            int t = 0;
        }
        p[i] = grid->getCoordinatesFromVoxelIndex(get<0>(indexes_of_vertexes[i]), get<1>(indexes_of_vertexes[i]), get<2>(indexes_of_vertexes[i]));
