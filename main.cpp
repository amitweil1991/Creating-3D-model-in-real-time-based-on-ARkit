 ...
 */
void testPresentTwentyFiveRaysFromFrames(vector<Matx31f>& cam_poses, vector<vector<worldPoint>>& frames_featurs, grid3D& grid) {
    vector<vector<tuple<int, int, int>>> keys;
    for (int i = 0; i < 50/*frames_featurs.size()*/; i++) {
//        cout << "frame num " << i << endl;
        for (int j = STARTING_FEATURE_TO_PRESENT; j <  FEATURES_NUM_TO_PRESENT /*frames_featurs[i].size()*/; ++j) {
...
//    Matx31f coordinates(grid.getCoordinatesFromVoxelIndex(get<0>(indexs), get<1>(indexs), get<2>(indexs)));
...
void exportGridToFile(grid3D& grid){
    ...
    for (auto it = grid.getGrid().begin(); it != grid.getGrid().end() ; it++) {
        if(it->second.getConfidence() >= THRESHOLD_CONFIDENCE) {
            ...
            grid.getGrid().erase(it);
        }
    }
    cout << i << endl;
    cout << "grid_size  " << grid.getGrid().size() << endl;
    outFile.close();
}
...
//void testCompareTuples(){
//    vector<tuple<int,int,int>> one;
//    vector<tuple<int,int,int>> two;
//    for(int i = 0; i < 5; i++) {
//        tuple<int, int, int> tup_1(i, i + 1, i + 2);
//        one.push_back(tup_1);
//        two.push_back(tup_1);
//    }
//    gridCellKey key_1(one);
//    map<gridCellKey, int> map;
//    map.insert(pair<gridCellKey, int>(key_1, 1));
...
//    testPresentTwentyFiveRaysWithSameFeature(cam_poses, frames_features, grid);
    testPresentTwentyFiveRaysFromFrames(cam_poses, frames_features, grid);
...
//    cout << " start create full mesh" << endl;
    ...
    /// export mesh to file
    string path_to_file("grid_as_mesh.off");
